...
class DownSample(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("DownSampleconv1", x.shape, out.shape, self.conv1.kernel_size, self.conv1.stride)
        #shape1 = out.shape
        out = F.relu(self.conv2(out))
        #print("DownSampleconv2", shape1, out.shape, self.conv2.kernel_size, self.conv2.stride)
        return out
...
class UpSample(nn.Module):
    ...
    def forward(self, x, output_size):
        ...
        #print("UpSampledeconv1", x.shape, out.shape, self.deconv.kernel_size, self.deconv.stride)
        #shape1 = out.shape
        out = F.relu(self.conv(out))
        #print("UpSampleconv2", shape1, out.shape, self.conv.kernel_size, self.conv.stride)
        return out
...
class GridDehazeNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape,x_index[0][0].shape)
        x_index = [[0 for _ in range(self.width)] for _ in range(self.height)]
        i, j = 0, 0
        inp=torch.zeros(x.shape[0],self.depth_rate,x.shape[2],x.shape[3]).cuda()
        inp[:,0:3,:,:]=x
        x_index[0][0]=inp
        ...
        #print("convout", shape1, out.shape, self.conv_out.kernel_size, self.conv_out.stride)
